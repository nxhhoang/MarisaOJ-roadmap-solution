...
struct DSU {
    ...
    DSU (int n) {
        ...
        for (int i = 1; i <= n; i++) vec[i] = i, st[i] = 1;
    }
...
ll calc() {
    ...
    for (auto [w, u] : ok) {
        ...
        for (auto ve : tre[u]) if (vis[ve]) st[tmp.find_set(ve)] = tmp.cc(ve);
        for (auto ve : tre[u]) if (vis[ve]) tmp.union_set(ve, u);
        for (auto it : st) res += w * it.second, sum += it.second;
        
        sum = sum * sum;
        for (auto it : st) sum -= it.second * it.second;
...
void solve()
{
    ...
    for (int i = 1; i <= n; i++) ok.push_back({arr[i], i});
    sort(all(ok));
    ll ans = calc();
