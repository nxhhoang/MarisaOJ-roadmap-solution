...
void solve() {
    ...
    sort(all(sortt));
    int ptr = 0;
    FOR(i, 0, n) roirac[sortt[i].second] = ptr++;
    ...
    FOR(i, 0, n) {
        ...
        value[i] = (maxV = max(maxV, st[find_set(roirac[y])]));
    }
    int a;
    pii cont;
    while(q--) {
        ...
        else if (a >= sortt[n - 1].first) cout << value[n - 1] << endl;
        else {
            cont = {a, INT_MAX};
            auto it = upper_bound(all(sortt), cont);
            if (it != sortt.begin()) --it;
            cout << value[it - sortt.begin()] << endl;            
        }
    }
}
