...
void solve()
{
    ...
    for (int len = 2; len <= n; len++) {
        for (int l = 1; l + len - 1 <= n; l++) {
            int r = l + len - 1;
            if (s[l] == '0' && t[l] == '1') dp[l][r] = dp[l + 1][r] + 1;
            if (s[r] == '0' && t[r] == '1') dp[l][r] = min(dp[l][r], dp[l][r - 1] + 1);
            if (s[l] == '?') dp[l][r] = min(dp[l][r], dp[l + 1][r] + 1);
            if (s[r] == '?') dp[l][r] = min(dp[l][r], dp[l][r - 1] + 1);
            if (s[l] == '?' && s[r] == '?') dp[l][r] = min(dp[l][r], dp[l + 1][r - 1] + 2);
            int val = '1' + '0';
            if (s[l] != s[r] && s[l] + t[l] == val && s[r] + t[r] == val) 
                dp[l][r] = min(dp[l][r], dp[l + 1][r - 1] + 1);
            if (s[l] == '?' && s[r] != '?') {
                if (s[r] == '1' && t[l] == '1' && t[r] == '0') dp[l][r] = min(dp[l][r], dp[l + 1][r - 1] + 2);
                if (s[r] == '0' && t[l] == '0' && t[r] == '1') dp[l][r] = min(dp[l][r], dp[l + 1][r - 1] + 2);
            }
            if (s[l] != '?' && s[r] == '?') {
                if (s[l] == '0' && t[l] == '1' && t[r] == '0') dp[l][r] = min(dp[l][r], dp[l + 1][r - 1] + 2);
                if (s[l] == '1' && t[l] == '0' && t[r] == '1') dp[l][r] = min(dp[l][r], dp[l + 1][r - 1] + 2);
            }
            for (int k = l; k < r; k++) {
