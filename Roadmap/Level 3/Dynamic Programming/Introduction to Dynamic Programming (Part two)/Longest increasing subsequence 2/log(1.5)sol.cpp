...
inline int log32(int x) {return log((double)x)/log(1.5f);} 
...
void solve()
{
    ...
    int m = n - cnt;
    vector<int> che(log32(m) + 5);
    int d = 0;
    double l5 = 3;
    double l4 = 2;
    che[d++] = 0;
    while (l5 <= m * l4) che[d++] = (l5 / l4), l5 *= 3, l4 *= 2; //, cout << che[d - 1] << " ";
    // cout << endl;
    if (che[d - 1] != m - 1) che[d++] = m - 1;
    // for (int i = 0; i < d; i++) cout << che[i] << " ";
    // cout << endl;
    // return;

    for (int i = 0; i < pos.size(); i++) {
        int val = 0, cnt = pos[i];
        int j = che[log32(cnt + 1)]; 
        
        // cout << j << endl;
        if (cnt == j && log32(i + 1) > 0) j = che[log32(cnt + 1) - 1];
        ...
        // cout << "--" << endl;
        // for (int h = 0; h < brr.size(); h++) cout << brr[h] << " ";
        // cout << endl;
        // for (int h = 0; h < m; h++) cout << dp[h] << " "; 
        // cout << endl;
    }
