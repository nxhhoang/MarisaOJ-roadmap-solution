...
struct Node{
    ...
    Node* left;
    Node* right;
    Node() : data(INT_MAX), cnt(0), left(nullptr), right(nullptr) {}
    // Node() : data(INT_MAX), cnt(0), val(INT_MAX), left(nullptr), right(nullptr) {}
};

void addNode(Node* root, int i, int val) {
    if (i == 30) {
    ...
    if (((val >> (29 - i)) & 1) == 0) {
        ...
        root->left->cnt++, root->left->data = val;
        addNode(root->left, i + 1, val);
    } else { 
        if (root->right == nullptr) root->right = new Node();
        root->right->cnt++, root->right->data = val;
        addNode(root->right, i + 1, val);
    }

    if (root->left && root->right == nullptr) {
        if (root->left->cnt == 1) root->data = root->left->data;
        else root->data = root->left->data;
        return;
    }

    if (root->right && root->left == nullptr) {
        if (root->right->cnt == 1) root->data = root->right->data;
    ...
    else if (root->left->cnt == 1 && root->right->cnt == 1) root->data = root->left->data ^ root->right->data;
...
void removeNode(Node* root, int i, int val) {
    if (i == 30) {
        if (root->cnt == 0) root->data = INT_MAX;
    ...
    if (((val >> (29 - i)) & 1) == 0) {
        root->left->cnt--;
        removeNode(root->left, i + 1, val);
    } else {
        root->right->cnt--;
        removeNode(root->right, i + 1, val);
    } 
