...
void solve() {
    ...
    while(q.size()) {
        ...
        for (int i = 0; i < tre[top].size(); i++) {
            ...
            if (tre[top][i].first == 1 && dis[top] + 1 == maxD) {
                vsec[1].push_back({tre[top][i].second, top});
                continue;
            }
            ...
            if (loaibo.find(tre[top][i].first) == loaibo.end()) 
                vsec[tre[top][i].first].push_back({tre[top][i].second, top});
        }
    }
    cout << maxD << endl;
    for (int i = 1; i < n + 1; i++) {
    ...
    queue<vector<pii>> qset;
    queue<vector<pii>> temp;
    qset.push(vsec[1]);
    for (int i = 0; i < maxD; i++) {
        ...
        for (int j = 0; j < val; j++) {
            ...
            for (auto tmp = it.begin(); tmp != it.end(); ++tmp) {
                if (tmp->first == minV && vis[tmp->second] == 0) qset.push(vsec[tmp->second]), vis[tmp->second] = 1;
            }
        }
        cout << minV << " ";
    }
