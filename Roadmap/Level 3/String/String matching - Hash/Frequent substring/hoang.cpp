...
vector<ll> ohass(MSIZE, 0);
vector<ull> base(MSIZE, 1);
vector<ll> obase(MSIZE, 1);
ull vbase = 331;
ll oprim = 337;
string s;
int k ;
...
ull query(int l, int r) {
    return hass[r] - hass[l] * base[r - l] + (ohass[r] - ohass[l] * obase[r - l] + MOD * MOD) % MOD;
}
...
void solve()
{
    ...
    for (int i = 1; i <= s.size(); i++) {
        ...
        obase[i] = obase[i - 1] * oprim % MOD;
        ohass[i] = (ohass[i - 1] * oprim + s[i - 1]) % MOD;
    }
